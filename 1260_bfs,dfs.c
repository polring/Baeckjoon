...
int graph[MAX_VERTICES][MAX_VERTICES] = { 0, };  //2차원 배열 초기화 
...
void dfs(int v, int vertices) 
{
    ...
    DFS_V[v] = 1;
    printf("%d ", v);
    for (w = 1; w <= vertices; w++) //문자열 순회하기
    {
        if (graph[v][w] == 1 && DFS_V[w] == 0)  //dfs를 통해서 그래프 탐색 
        {
            dfs(w, vertices);
        }
    }
...
void bfs(int v, int vertices) 
{
    ...
    while (front < rear) 
    {
        pop = queue[front]; front++; //큐에서 값 꺼내기
        for (w = 1; w <= vertices; w++)  //문자열을 순회하면서 값 찾기 
        {
